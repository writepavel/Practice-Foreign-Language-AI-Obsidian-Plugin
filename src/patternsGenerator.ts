...
async function generateGrammarPatterns(plugin: IPracticeForeignLanguagePlugin): Promise<void> {
    ...
    const sourceFileLink = activeFile ? `[[${activeFile.path}]]` : "Unknown source";
...
Generated by search query ${sourceFileLink}
    `;
    ...
    function createPrompt(context: any): string {
        ...
        return JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "You are a professional creative teacher of Czech language with 10 years of experience that generates Czech language grammar patterns for language learning but that are commonly used and can be often heard in the Czech environment. Always return your response as a valid JSON array, strictly adhering to the specified grammar requirements. The Czech, Russian, and English fields are absolutely mandatory and must be provided for every pattern. Ensure all quotes within string values are properly escaped with a backslash (\\)."
              },
              {
                role: "user",
                ...
                - The "czech", "russian", and "english" fields are MANDATORY for every pattern. Do not omit these under any circumstances.
              ...
              Return the result as a JSON array of objects, each with the following structure:
              {
                ...
                "english": "English translation", // MANDATORY
                "grammar": {
              ...
              Remember, the "czech", "russian", and "english" fields are absolutely mandatory for each pattern.
              Ensure all quotes within string values are properly escaped with a backslash (\\).`
          }
          ]
        });
